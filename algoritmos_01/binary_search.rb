...
def search(array, x)
  ...
  half = array.sort!.length / 2 # metade do array e ordena o array
  ...
  if middle == x   # está no meio do array ?
    ...
    puts "O elemento #{x} está presente no índice #{array.find_index(x)} --- (primera metade)"

  elsif array[half..-1].include?(x) # está na segunda metade do array?
    puts "O elemento #{x} está presente no índice #{array.find_index(x)} --- (segunda metade)"
  else
